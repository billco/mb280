...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    if (numRead > 0) {
      if (buf[0] == 'q') {
	break;
      } else if (buf[0] == 's') {	// hide off scan sensors
	...
	if (!hideOffScan)
      ...
      printf
	("  Cur       LOW        HI        AV       Scal        Raw  Bme:%d %s%s\n",
	...
	printf("%s", DIMWHITE);
      ...
      printf
	("Temp : %6.1f    %6.1f    %6.1f    %6.1f     %6.1f     %6.1f  lop:%d\n",
	 ((float) data[TEMP]) * 0.1, ((float) data[MTEMP]) * 0.1,
	 ((float) data[XTEMP]) * 0.1, ((float) data[ATEMP]) * 0.1,
	 ((float) sCal[STEMP]) * 0.1, ((float) data[RTEMP]) * 0.1, getLoop(ix));

      printf
	("Hum  : %6.1f    %6.1f    %6.1f    %6.1f     %6.1f     %6.1f  adr:%x\n",
	 ((float) data[HUM]) * 0.1, ((float) data[MHUM]) * 0.1,
	 ((float) data[XHUM]) * 0.1, ((float) data[AHUM]) * 0.1,
	 ((float) sCal[SHUM]) * 0.1, ((float) data[RHUM]) * 0.1, getAdr(ix));
      printf
	("Pres : %6.1f    %6.1f    %6.1f    %6.1f     %6.1f     %6.1f  log:%2d\n",
	 ((float) data[PRES]) * 0.1, ((float) data[MPRES]) * 0.1,
	 ((float) data[XPRES]) * 0.1, ((float) data[APRES]) * 0.1,
	 ((float) sCal[SPRES]) * 0.1, ((float) data[RPRES]) * 0.1,
