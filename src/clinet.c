...
void ssdisplay(int ix);
void aldisplay(int ix);
...
void aldisplay(int ix)
{
  ...
  printf
    ("  Cur       LOW         HI     Alarm      Scal        Raw  Bme:%d %s%s\n",
     ix, DEFCOLOR, UNDERLINEOFF);

  if (!IsOnScan(ix)) {
    printf("%s", DIMWHITE);
  }
  alarmStat = data[ALSTAT] & 0x0000FFFF;

  printf
    ("Temp : %6.1f    %6.1f %3s %6.1f %3s  %04X    %6.1f     %6.1f  lop:%d\n",
     ((float) data[TEMP]) * 0.1,
     ((float) sCal[ASP_TLO]) * 0.1, strIsLoTempAct(alarmStat),
     ((float) sCal[ASP_THI]) * 0.1, strIsHiTempAct(alarmStat),
     alarmStat & ATBITS,
     ((float) sCal[STEMP]) * 0.1, ((float) data[RTEMP]) * 0.1,
     getLoop(ix));

  printf
    ("Hum  : %6.1f    %6.1f %3s %6.1f %3s  %04X    %6.1f     %6.1f  adr:0x%2x\n",
     ((float) data[HUM]) * 0.1,
     ((float) sCal[ASP_HLO]) * 0.1, strIsLoHumAct(alarmStat),
     ((float) sCal[ASP_HHI]) * 0.1, strIsHiHumAct(alarmStat),
     alarmStat & AHBITS,
     ((float) sCal[SHUM]) * 0.1, ((float) data[RHUM]) * 0.1, getAdr(ix));
  printf
    ("Pres : %6.1f    %6.1f %3s %6.1f %3s  %04X    %6.1f     %6.1f  log:%2d\n",
     ((float) data[PRES]) * 0.1,
     ((float) sCal[ASP_PLO]) * 0.1, strIsLoPresAct(alarmStat),
     ((float) sCal[ASP_PHI]) * 0.1, strIsHiPresAct(alarmStat),
     alarmStat & APBITS,
     ((float) sCal[SPRES]) * 0.1,
     ((float) data[RPRES]) * 0.1, (IsOnScan(ix) ? data0[8] : 0));
