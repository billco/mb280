...
archist(void)
{
	...
	if (file_exists(buf)) {
	    sprintf(buf1, "./data/arc/%02d%02d%02dhist%d.csv",
		    tm.tm_mon + 1, tm.tm_mday, tm.tm_year - 100, x);
	    rename(buf, buf1);
	}
    }
}
