...
//setlogmask (LOG_UPTO (LOG_NOTICE));
 ...
 void resetHum( int ix ){
	b[ix].data[MHUM] = 999; b[ix].data[XHUM] = b[ix].data[AHUM] = 0;
		printf("reset %d hum\n", ix);
 ...
 void resetTemp( int ix){
	...
	b[ix].data[MTEMP] = 999; b[ix].data[XTEMP] = -9999; b[ix].data[ATEMP] = 0;
		printf("reset %d temp\n", ix);
}

 void resetPres( int ix){
	b[ix].data[MPRES]  = 100;  b[ix].data[XPRES] = -9999; b[ix].data[APRES] = 0;
		printf("reset %d pres\n", ix);
 ...
 //printf("activeId = %d\n", activeId );
                   //	activeId = 0;
                   	...
                   	if( activeId >= 0 && activeId <= MAXLP) {
						...
						memcpy( mb_mapping->tab_input_registers, b[activeId].sCal, LASTS << 1 );
					}
                   ...
                   if( query[COMMAND] == FUNC6  ){
						...
						syslog (LOG_NOTICE,"Client:%d Id:%d Write reg:%d val:%d\n",
