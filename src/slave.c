...
void resetHum(int ix)
{
  b[ix].data[MHUM] = 999;
  b[ix].data[XHUM] = b[ix].data[AHUM] = 0;
  printf("reset %d hum\n", ix);

}

void resetTemp(int ix)
{

  b[ix].data[MTEMP] = 999;
  b[ix].data[XTEMP] = -9999;
  b[ix].data[ATEMP] = 0;
  printf("reset %d temp\n", ix);
}

void resetPres(int ix)
{
  b[ix].data[MPRES] = 100;
  b[ix].data[XPRES] = -9999;
  b[ix].data[APRES] = 0;
  printf("reset %d pres\n", ix);
...
static void stats(int ix)
{
  // find max min av
  //if( cur == 0 ){
  //printf( "min:%d, temp: %d %d\n", b[cur].data[MTEMP] , b[cur].data[TEMP] ,
  //( b[cur].data[MTEMP] < b[cur].data[TEMP]));
  //}
  if (b[ix].data[TEMP] > b[ix].data[XTEMP])
    b[ix].data[XTEMP] = b[ix].data[TEMP];
  if (b[ix].data[MTEMP] >= b[ix].data[TEMP])
    b[ix].data[MTEMP] = b[ix].data[TEMP];
  b[ix].data[ATEMP] = (b[ix].data[MTEMP] + b[ix].data[XTEMP]) >> 1;

  if (b[ix].data[HUM] > b[ix].data[XHUM])
    b[ix].data[XHUM] = b[ix].data[HUM];
  if (b[ix].data[MHUM] >= b[ix].data[HUM])
    b[ix].data[MHUM] = b[ix].data[HUM];
  b[ix].data[AHUM] = (b[ix].data[MHUM] + b[ix].data[XHUM]) >> 1;

  if (b[ix].data[PRES] > b[ix].data[XPRES])
    b[ix].data[XPRES] = b[ix].data[PRES];
  if (b[ix].data[MPRES] < b[ix].data[PRES])
    b[ix].data[MPRES] = b[ix].data[PRES];
...
static void gager(void)
{
  ...
  while (gagerRun) {
	...
	b[cur].data[PRES] = b[cur].data[RPRES] + b[cur].sCal[SPRES];
...
static void Slave(void)
{
  ...
  mb_mapping = modbus_mapping_new(MODBUS_MAX_READ_BITS, 0, LASTREG, LASTS);
  if (mb_mapping == NULL) {
  ...
  for (;;) {
	...
	if (rc > 0) {
	  ...
	  if (activeId >= 0 && activeId <= MAXLP) {
	    ...
	    memcpy(mb_mapping->tab_input_registers,
		   b[activeId].sCal, LASTS << 1);
	  }
